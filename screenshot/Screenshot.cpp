...
void Screenshot::newScreenshot()
{
	...
	//begin to add test code
	static QString s_srcImagePath = "D:\\works\\project\\AiSprite\\conanVersion\\aisprite\\Win32\\TestData\\SrcImages";
	QScreen *scr = QGuiApplication::primaryScreen();
	QList<QScreen *> dd = QGuiApplication::screens();
	QTimer *captureScreenTimer = new QTimer(this);
	captureScreenTimer->setInterval(1000);
	QEventLoop loop;
	connect(captureScreenTimer, &QTimer::timeout, this, [&loop, scr, captureScreenTimer]() {
		...
		QDir filePath(s_srcImagePath);
		if(!filePath.exists())
			filePath.mkpath(".");

		QFile file(s_srcImagePath + "\\" + QString("srcImage%1.png").arg(nth++));
